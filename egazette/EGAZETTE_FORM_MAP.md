...
- Update: custom UA/forced-Referer headers caused HTTP-200 "Runtime Error" pages on fresh entry; browser-like UA with no forced Referer returns HTTP 200 shells instead (client fixed). But the live directory shell (20806 bytes) currently renders NO form controls even via warmed Default.aspx -> href navigation, while the 05:27 sample had the full Category/Part/Year form and the 2025 partition enumerated 45 records. So the directory route is server-side unavailable since ~11:45, not a client flow bug. Bill stream (reftype 9) also lacks the 48-of-2023 record (only the 2018 amendment bill).
- Entry notes (2026-09-25): site home is `Default.aspx` (not `home.aspx`, which YSODs); live `SearchMenu.aspx` carries the 8 `btn*` buttons with only a `Default.aspx` href — directory entry is via the home menu href chain, untested while the host is timing out. Host oscillates between fast and all-endpoint timeouts within minutes; keep every probe single-shot and back off on clusters of timeouts.
- Degraded mode (2026-09-25 ~12:30+): postback-rendered result pages return the 20806-byte unrendered shell (0 grid buttons, no alert) even for queries that succeeded minutes earlier. Signature distinguishes it from genuine "No Gazette Found". fetch --dry-run doubles as a health probe: records: 0 + 20806-byte page = host degraded, not a code bug.
- Escalation: the 20806-byte shell now covers even SearchMenu.aspx (no btnBill rendered) after root-warmed entry — the app is not rendering any postback content, so all SearchBill/directory flows are down together until the server recovers. Ruled out: headers (fixed), entry path (warmed Default->href tested), query text (known-good queries fail identically).
